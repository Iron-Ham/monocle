...
public actor SourceKitService {
  ...
  public func start(workspace: Workspace, toolchain: ToolchainConfiguration?) async throws -> InitializingServer {
    ...
    let channelWithProcess: (channel: DataChannel, process: Process) = try DataChannel.localProcessChannel(parameters: parameters) { [weak self] in
    ...
    let initializeProvider: InitializingServer.InitializeParamsProvider = { [workspace] in
      ...
      return InitializeParams(
        ...
        clientInfo: InitializeParams.ClientInfo(name: "monocle", version: "0.1.0"),
  ...
  private func makeExecutionParameters(workspace: Workspace, toolchain: ToolchainConfiguration?) throws -> Process.ExecutionParameters {
    ...
    if workspace.kind == .swiftPackage {
      ...
      environment["SWIFTPM_CACHE_PATH"] = URL(fileURLWithPath: workspace.rootPath).appendingPathComponent(".swiftpm-cache").path
    }
